...
import com.mcreater.amcl.controls.items.ListButtonItem;
...
public class LocalModInfoDialog extends AbstractDialog {
    ...
    Label translate;
    private final boolean enableTranslate;
    ...
    public LocalModInfoDialog(LocalMod c, boolean trans) {
        ...
        links.title.setOnAction(event -> {
            if (links.cont.getSelectionModel().getSelectedItem() != null) {
                SystemActions.openBrowser(links.cont.getSelectionModel().getSelectedItem().url);
            }
        });
        ...
        layout.setHeading(new VBox(title, translate));
        layout.setBody(pane);
    ...
    private void loadLinks() {
        if (this.model != null) {
            ...
            links.cont.getSelectionModel().selectFirst();
            if (this.model.name.cn != null && enableTranslate) translate.setText(this.model.name.cn);
        }
    }
