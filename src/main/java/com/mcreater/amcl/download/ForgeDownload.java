...
public class ForgeDownload {
    ...
    public static void download(String id, String minecraft_dir, String version_name, int chunkSize, NewForgeItemModel forge_version, Runnable r3, Runnable r, Runnable r2, FasterUrls.Server server) throws Exception {
        ...
        if (new File(versionPath).exists()) {
            ...
            tasks.forEach(task -> {
                if (task instanceof AbstractDownloadTask)
                    ((AbstractDownloadTask) task).setType(AbstractDownloadTask.TaskType.FORGE);
            });
    ...
    public static void download_mojmaps(String local, FasterUrls.Server server) throws IOException {
