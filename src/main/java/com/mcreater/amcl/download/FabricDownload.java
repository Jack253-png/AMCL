...
import static com.mcreater.amcl.util.net.FasterUrls.FABRIC_LOADER_FORMAT;
...
public class FabricDownload {
    ...
    public static void download(String id, String minecraft_dir, String version_name, int chunkSize, String fabric_version, Runnable ru, FasterUrls.Server server) throws Exception {
        tasks.clear();
        ...
        ru.run();
        String fab = FasterUrls.fast(String.format(FABRIC_LOADER_FORMAT, id, fabric_version), server);
        ...
        try {
            ...
            url = FasterUrls.fast(FABRIC_MAVEN_ROOT + MavenPathConverter.get(model.loader.maven).replace("\\", "/"), server);
            ...
            for (FabricLibModel s : model.launcherMeta.libraries.common) {
                String url = FasterUrls.fast(J8Utils.requireNonNullElse(s.url, FasterUrls.ORIGINAL_LIB_ROOT) + MavenPathConverter.get(s.name).replace("\\", "/"), server);
            ...
            String url = FasterUrls.fast(FABRIC_MAVEN_ROOT + MavenPathConverter.get(model.loader.maven).replace("\\", "/"), server);
            ...
            String url1 = FasterUrls.fast(FABRIC_MAVEN_ROOT + MavenPathConverter.get(model.intermediary.maven).replace("\\", "/"), server);
